...
In the Hamiltonian formalism, \textbf{our first task is always to determine the Hamiltonian $H$.}
$$
H=p \dot{x}-L
$$
...
x^{2}+y^{2}=l^{2}
$$
\textbf{\bluep{In Newtonian formalism}}, we have the equation of motion as:
...
\textbf{\bluep{In Lagrangian formalism}}, we add the Lagrange multiplier term in the Lagrangian:
\begin{equation}
\begin{aligned}
...
\textbf{\bluep{In Hamiltonian formalism}}, we again first calculate the generalized momentum $p$ as:
$$
\begin{aligned}
...
In a nutshell, \textbf{\redp{Noether’s theorem tells us that for each symmetry of a given system, there must be a conserved quantity}}
...
In contrast, if there is no potential:
$$
...
\end{equation}
\end{qt}
\redp{\textbf{We therefore learn that if G generates a symmetry, it automatically
describes a conserved quantity.}This is Noether's theorem.}

\subsection{Noether's extended theorem}
We have learned that \redp{gauge transformation} have no influence on Hamilton's equations and are therefore always sysmmetries. Therefore, it seems reasonable to relax the condition (\ref{hamilinvariant}) to:
\begin{qt}
    \begin{equation}
...
\subsection{In Lagrangian Formalism}
...
But this means that \textbf{we can try to find a\textbf{ Hamilton's principal function} $W=W(q, Q, t)$ for which the Hamiltonian becomes zero:}
\begin{equation}
...
\end{equation}
\end{qt}
Using Liouville equation, we can derive the \redp{Liouville's theorem}
\begin{qt}
\begin{center}
    Phase space volume remains constant in time.
\end{center}
\end{qt}
\subsection{Koopmann-von Neumann Mechanics}
...
\end{center}
\end{qt}
\subsubsection{Koopman-von Neumann Equation}
...
We first multiply Eq.(\ref{k-veqn}) by the conjugate state vector
\begin{equation}
...
To calculate the total probability that our system evolves from a specific initial point $A$ to a specific final point $B$ we sum over the probability amplitudes for all possible paths:
$$
...
\end{equation}
\end{qt}
In general, since the action for a specific path $(S[q(t)])$ is an ordinary number, $e^{i S}[q(t)]$ is a complex number with absolute value $1 .$ In the complex plane, these numbers lie as vectors on the unit circle. \redp{The contribution of each path to the total path integral is therefore simply a unit complex number.} To calculate the path integral, we have to \redp{add the little arrows for each path like we would add vectors from complex plane. The total value of the path integral is then the resulting arrow}.
